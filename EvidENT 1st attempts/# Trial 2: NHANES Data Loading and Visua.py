# Trial 1: NHANES Data Loading and Visualization (Finished Version)
# Author: Isabella
# Date: 2025-09-15
...
# -----------------------------
# Utility: Pick a CSV file
# -----------------------------
def find_csv(folder, prefix):
    """Find the first CSV in folder starting with prefix."""
...
def filter_aux_df(nhanes_aux_df):
    ...
    available = [c for c in cols if c in nhanes_aux_df.columns]
    return nhanes_aux_df[available].replace({888: np.nan, 666: np.nan})

def filter_demo_df(nhanes_demo_df):
    cols = ['SEQN', 'RIAGENDR', 'RIDAGEYR', 'RIDAGEMN', 'RIDRETH1']
    available = [c for c in cols if c in nhanes_demo_df.columns]
    return nhanes_demo_df[available].reset_index(drop=True)
...
def load_and_filter_cohort():
    ...
    filtered_demo = filter_demo_df(demo_df)
