# Trial 1: NHANES Data Loading and Crazy Visualizations
# Author: Isabella
# Date: 2025-09-15
...
ENABLE_SEQN_FILTER = False  # Keep False until SEQNs match
...
# -----------------------------
# 3D Hearing Landscape
# -----------------------------
freq_map_3d = {
    '500': 'AUXU500R', '1K1': 'AUXU1K1R', '1K2': 'AUXU1K2R',
    '2K': 'AUXU2KR', '3K': 'AUXU3KR', '4K': 'AUXU4KR',
    '6K': 'AUXU6KR', '8K': 'AUXU8KR'
}

freq_cols = [col for col in freq_map_3d.values() if col in aux_df.columns]
freq_values = [int(col[4:-1].replace('K','000')) for col in freq_cols]
...
# -----------------------------
# Hearing Fingerprint (Radar Plot)
# -----------------------------
def radar_plot(seqn):
    participant = aux_df[aux_df['SEQN'] == seqn]
    if participant.empty:
        print(f"❌ No participant with SEQN {seqn}")
        return

    categories = [c for c in aux_df.columns if c.endswith('R') and c != 'RIDAGEYR']
    values = participant[categories].values.flatten().tolist()
    values += values[:1]
    ...
    ax.plot(angles, values, linewidth=2, linestyle='solid', label=f'SEQN {seqn}')
    ax.fill(angles, values, 'green', alpha=0.25)
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels([c[4:-1] for c in categories], fontsize=8)
    ax.set_title(f"Hearing Fingerprint for SEQN {seqn}", y=1.08)
    plt.legend(loc='upper right')
    plt.show()

# Plot radar plots for first 3 participants
for seqn in aux_df['SEQN'].head(3):
    radar_plot(seqn)
