...
demo_folder = None
for folder in possible_demo_folders:
    full_path = os.path.join(base_dir, folder)
    if os.path.exists(full_path) and os.path.isdir(full_path):
        demo_folder = full_path
        print(f"✅ Using demographics folder: {folder}")
        break

if demo_folder is None:
    raise FileNotFoundError("❌ Could not find any demographics folder in CSV DATA.")
...
# ----------------------------
df = demo.copy() if not demo.empty else pd.DataFrame()
...
# ----------------------------
# 7. Define hearing loss outcome safely
# ----------------------------
# Detect available audiometry columns
available_au_cols = [c for c in audiometry_features if c in df.columns]
print("Available audiometry columns:", available_au_cols)

left_cols  = [c for c in available_au_cols if c.endswith("K1")]
right_cols = [c for c in available_au_cols if c.endswith("K2")]

if not left_cols or not right_cols:
    raise ValueError("No audiometry columns found for left or right ear. Check CSV files.")

# Compute PTA and hearing loss
df["pta_left"]  = df[left_cols].mean(axis=1, skipna=True)
df["pta_right"] = df[right_cols].mean(axis=1, skipna=True)
